...
static PSG* createPSG(int psgClock, int sampleRate)
{
  ...
  //PSG_setVolumeMode(psg, EMU2149_VOL_YM2149);  // AY-3-8910 mode
  //PSG_set_quality(psg, 1);   // Quality = low?
  PSG_reset(psg);
  return psg;
}
 ...
 */
void __noinline __time_critical_func(audioUpdate)()
{
  // generate audio data
  PSG_calc(psg0);
  PSG_calc(psg1);
  ...
  //uint16_t leftAudio  = ((abs((int32_t)psg0->ch_out[0] + (int32_t)psg0->ch_out[1] + (int32_t)psg0->ch_out[2])) * PWM_WRAP) >> 13;
  //uint16_t rightAudio = ((abs((int32_t)psg1->ch_out[0] + (int32_t)psg1->ch_out[1] + (int32_t)psg1->ch_out[2])) * PWM_WRAP) >> 13;
