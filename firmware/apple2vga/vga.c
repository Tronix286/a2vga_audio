...
//    sm_config_set_clkdiv(&c, CONFIG_SYSCLOCK / (2 * PIXEL_FREQ));  // 2 * PIXEL_FREQ
    sm_config_set_clkdiv(&c, CONFIG_SYSCLOCK / (4*PIXEL_FREQ)); //was 2*pixelclk

    // Map the state machine's OUT pin group to the data pins
    sm_config_set_out_pins(&c, CONFIG_PIN_RGB_BASE, 5);
    sm_config_set_set_pins(&c, CONFIG_PIN_RGB_BASE, 5);

// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    sm_config_set_sideset_pins(&c, CONFIG_PIN_LATCH);
    ...
    // Configure the pins as outputs & connect to the PIO
    pio_sm_set_consecutive_pindirs(pio, sm, CONFIG_PIN_RGB_BASE, 6, true);
    for(int i = 0; i < 6; i++) {
